...
**fga-ts** is a TypeScript library that implements OpenFGA-compatible
...
fga-ts is a direct port of pgfga's recursive check algorithm. The 5-step check
logic in `src/core/check.ts` must faithfully reproduce the behavior of pgfga's
`pgfga.check()` function. Conformance is validated by running both fga-ts and a
...
```
Public API (createFga) → Core Algorithm (check, conditions) → TupleStore interface → Kysely adapter
```
...
Creates the `fga` schema with 3 tables and 6 indexes matching pgfga's schema
design.
...
-- fga-ts indexes (object_type, object_id, relation) to match the forward lookup in step 2.
...
- Added `condition_name`, `condition_context` columns to `fga.tuples`
- Added `fga.condition_definitions` table
- Schema is `fga` (not `pgfga`)
...
```typescript
export function createFga(store: TupleStore, options?: CheckOptions): FgaClient;
...
Conformance tests validate that fga-ts produces identical results to a real
...
OpenFGA uses its own `openfga` schema in the same PostgreSQL instance. fga-ts
uses the `fga` schema. They share the database but not schema.
...
5. Run Kysely migrations to create `fga` schema
...
**Key difference from pgfga:** `expectConformance` receives an `FgaClient`
instance (not a raw SQL connection). The fga-ts check runs through the library's
own API.
...
import { createFga, KyselyTupleStore } from "../../src/index.ts";
import { expectConformance } from "../helpers/conformance.ts";
import { fgaCreateStore, fgaWriteModel, fgaWriteTuples } from "../helpers/openfga.ts";

describe("Slack Model Conformance", () => {
  let storeId: string;
  let fgaClient: FgaClient;

  beforeAll(async () => {
    // Setup fga-ts: create KyselyTupleStore, load relation configs + tuples
    const store = new KyselyTupleStore(db);
    fgaClient = createFga(store);
...
via fga-ts's own API (`fgaClient.writeRelationConfig()`, `fgaClient.addTuple()`).
...
bun test                                 # All tests (needs Docker)
...
- **No session context coupling.** pgfga uses `set_config`/`current_setting`
  for RLS. fga-ts always receives `subjectType` and `subjectId` explicitly in
...
- **Single config fetch for steps 3-5.** pgfga does 3 separate `SELECT` queries
  ...
  tuple_to_userset). fga-ts fetches the config once and reads all three fields
  from the result.
